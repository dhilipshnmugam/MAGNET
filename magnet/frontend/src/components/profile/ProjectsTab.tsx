...
import { Code, ChevronRight, Plus } from 'lucide-react';
...
export default function ProjectsTab({ projects, loading, isOwn }: ProjectsTabProps) {
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              ...
              <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium ${STATUS_COLORS[p.status || 'planning'] || STATUS_COLORS.planning}`}>
                {p.status}
              </span>
            </div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-gray-400">
              ...
              {p.my_role && <span className="capitalize">Role: {p.my_role}</span>}
            </div>
